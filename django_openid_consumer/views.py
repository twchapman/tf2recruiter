...
def begin(request, redirect_to=None, on_failure=None, template_name='openid_signin.html'):
    ...
    redirect_to = redirect_to or getattr(
        ...
        'http://twchapman.com/tf2recruiter/openid/complete/'
    )
...
def complete(request, on_success=None, on_failure=None, failure_template='openid_failure.html'):
    ...
    url = get_url_host(request) + '/tf2recruiter' + request.path
    ...
    if openid_response.status == SUCCESS:
        ...
        return on_failure(request, openid_response.message + ' <br /> ' + get_url_host(request) + ' <br />' + request.path, failure_template)
